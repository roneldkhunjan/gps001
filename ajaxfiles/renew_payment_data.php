...
$created_dt=(isset($_GET['created_dt']))?$_GET['created_dt']:'';
		...
		$r_invid=(isset($_GET['r_invid']))?$_GET['r_invid']:'';
...
$total_cash=$cashamount+$totlcash;
	...
	$cshq=mysql_query("INSERT INTO renew_cash (customer_id, order_id,r_invid, cashtype, cashamount, cashpending, totlpayablecash, cashpaid,total_cashamount) VALUES ('$cid', '$oid','$r_invid', '$cashtype', '$cashamount', '$cashpending', '$totlpayablecash', '$cashpaid','$total_cash')");
...
$total_cheque=$chequeamount+$totlcheque;
	...
	$chq=mysql_query("INSERT INTO renew_cheque (customer_id, order_id,r_invid, chequetype, chequeamount, chequepending, totlpayablecheque, chequepaid,chequeno,bankname,total_chequeamount,branchname,branchlocation) VALUES ('$cid', '$oid','$r_invid', '$chequetype', '$chequeamount', '$chequepending', '$totlpayablecheque', '$chequepaid','$chequeno','$bankname','$total_cheque','$branchname','$branchlocation')");
...
$chonq=mysql_query("INSERT INTO renew_online (customer_id, order_id,r_invid, onlinetype, pending) VALUES ('$cid', '$oid','$r_invid', '$onlinetype', '$totalpending')");
...
if($amount==$paidamount)

	{
		...
		$uq=mysql_query("update rpayment_list set status='done' where order_id='$oid' and r_invid='$r_invid'");

		$uq=mysql_query("update renew_invoice set status='paid' where order_id='$oid' and id='$r_invid'");
  ...
  <table width='600' border='0' cellspacing='0' cellpadding='10' style='background-color: rgb(224, 244, 255);''>
    ...
    <tr>
      ...
      <td align='left'>
		...
		<td>Thank you for your subscription renewal order for $submonth months.</td>
		...
		<td style='font-weight: bold;'>Order Number	: $oid</td>
...
";
	...
	$pyq=mysql_query("INSERT INTO rpayment_list (customer_id, order_id,r_invid, total_amount, paid_amount, pending_amount, status,cashamount,chequeamount,online_transfer_amount) VALUES ('$cid', '$oid','$r_invid', '$amount', '$paidamount', '$totalpending', '$status','$cashamount','$chequeamount','$onlinetransferamount')");
	...
	$raq=mysql_query("select * from renew_invoice where id='$r_invid'");

	$resaq=mysql_fetch_array($raq);

	$submonth=$resaq['sub_month'];

	$finalcost=$resaq['final_total'];

	

	//$raq=mysql_query("select * from installation where r_invoice='$r_invid'");

	//$resaq=mysql_fetch_array($raq);

	//$curdate=$resaq['final_total'];
...
<tr style='font-family: calibri;'>
  ...
  <table width='600' border='0' cellspacing='0' cellpadding='10' style='background-color: rgb(224, 244, 255);''>
	...
	<td>We would like to thank you for the Payment of Rs.$paidamount  for your order OD$oid renewal order for $submonth months.</td>
...
mail($to,$subject,$message,$headers);
		...
		$aqc=mysql_query("select * from admin_data where id='1'");
