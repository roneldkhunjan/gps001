...

{
		...
		$renewed_on=date("Y-m-d H:i:s");
		$ruaq=mysql_query("update r_installation set renew_approve_status='approved',renew_status='renewed',renewed_on='$renewed_on' where r_id=$r_id");
	...
	//	$ruq=mysql_query("update r_installation set renew_status='renewed',renew_month='$submonth' where r_id='$r_id'");
...

		{
			...
			$sql_i="select * from r_installation where r_id=$r_id";	
				$rst=mysql_query($sql_i);
				$ins=mysql_fetch_assoc($rst);

			$riq=mysql_query("INSERT INTO installation (customer_id, order_id, category_id, model_id, engineer_id, sim_no, device_id, imie_no, device_name, allocation_status, remarks, approve_status, installation_status, installed_date, installation_remarks, device_status, create_date_time, demo_device_type, allocated_date_time, company_id, image, speed_limit, idle_limit, device_date_time, assign_device_status, assign_engineer, submonth, order_date, customer_type, tank_capacity,formula,device_password,fuel_sensor,milege,sim_status) VALUES ('$customer_id', '$orderid', '$category_id', '$model_id', '$engineer_id', 'No Sim Chosen', '$device_id', '$imie_no', '$device_name', 'completed', '', '', 'completed', '$curdate', 'completed', 'active', CURRENT_TIMESTAMP, '$demo_device_type', '', '$company_id', 'noimage.jpg', '0', NULL, '', 'assigned', 'assigned', '$submonth', '$curdate','".$ins['customer_type']."','".$ins['tank_capacity']."','".$ins['formula']."','".$ins['device_password']."','".$ins['fuel_sensor']."','".$ins['milege']."','".$ins['sim_status']."')");

			$renewed_on=date("Y-m-d H:i:s");
			$ruaq=mysql_query("update r_installation set renew_status='renewed',renew_month='$submonth',renew_approve_status='approved',renewed_on='$renewed_on' where r_id='$r_id'");
