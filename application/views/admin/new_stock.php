...
<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
									...
									<?php 
									$slno=1;
								//	echo "select * from gps_model_details where status='pending' and first_status='pending' order by model_id desc";
									
									$q=mysql_query("select m.*,s.detail_id from gps_model_details m  left join stock_details s on m.imie_number=s.imei where m.first_status='pending' order by m.model_id desc");
...
<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
								...
								//	echo "select * from gps_model_details where status='pending' and first_status='pending' order by model_id desc";
									...
									$q=mysql_query("select * from gps_model_details where first_status='pending' order by model_id desc");
									while($res=mysql_fetch_array($q))
									{
								$devid=$res['device_id'];
								$catid=$res['category_type'];
