						...
						<?php	if(isset($msg2))
							...
							?>
								...
								<div class="infobox infobox-green  ">
										...
										<?php $cq= mysql_query("select sum(amt) as cnt from payment_master where response='success'");
										while($cres=mysql_fetch_array($cq))
										{
											$cnt=$cres['cnt'];
										}?>
											<span class="infobox-data-number"><?php echo $cnt;?></span>
											<div class="infobox-content">Online Account Balance</div>
										</div>
									</div>
...
<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
									<thead>
										...
										<!--	<th>Cash Balance</th>-->
											...
											<?php
											$slno=1;
											$q=mysql_query("SELECT cu.customer_uid,cu.customer_first_name,c.customer_id,c.final_cost,s.order_id,s.amt FROM  payment_master s left join customer_orders c on s.order_id=c.order_id left join customers cu on cu.customer_id=c.customer_id where s.response='success'");
