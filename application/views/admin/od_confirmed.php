...
$type=$user['user_type']; ?>
					...
					<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
						...
						$qo=mysql_query("Select * from customer_order_details o inner join payment_master s on s.order_id=o.order_id join customer_orders co on co.order_id=o.order_id where s.response='success'  and co.created_by='$mid' group by o.order_id order by co.order_id desc");
						while($srs=mysql_fetch_array($qo))
						{
							...
							$catid=$srs['category_id'];
								...
								$neq=mysql_query("Select o.order_id,sum(noofdevice) as ndev from customer_order_details o join customer_orders co on co.order_id=o.order_id where  o.customer_id='$id' and co.order_id='$order_id'  group by co.order_id  order by co.order_id desc");
