...
$mid=$user['id'];
$type=$user['user_type'];
if($type=='admin' || $type=='approver' || $type=='inventory')
{
?>	
					...
					<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
						...
						$qo=mysql_query("SELECT * FROM `customer_orders` co join customers c on c.customer_id=co.customer_id join payment_list p on p.order_id=co.order_id where approve_status='pending' and co.remarks='Waiting for Approval' order by co.appr_rej_date desc");
 ...
 <table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example2">
						...
						$qn=mysql_query("SELECT * FROM `customer_orders` co join customers c on c.customer_id=co.customer_id join payment_direcpay p on p.order_id=co.order_id and p.rinv=0 where p.final_status not in ('SUCCESS','FAIL') order by co.order_date desc");
 ...
 <td>                             
<form name="ecom" id="StotSForm<?php echo $i;?>" method="post" action="https://www.timesofmoney.com/direcpay/secure/dpPullMerchAtrnDtls.jsp" style=" margin:0px;" target="_blank">  
...
<!----> 
									...
									/////renewal payment..................
									$Res_URL="http://ossgpstracking.com/merchant_direcpay/StoSRResp.php";	
								$qn=mysql_query("SELECT r.id,r.customer_id,r.order_id,p.reference_id, c.customer_uid, c.customer_first_name, r.final_total,p.ts, p.amount, p.final_status FROM renew_invoice r join customers c on c.customer_id=r.customer_id join payment_direcpay p on p.rinv=r.id where p.final_status not in ('SUCCESS','FAIL') order by r.ts desc");
						while($rs=mysql_fetch_array($qn))
						{
							$custmainid=$rs['customer_id'];
							$order_id=$rs['order_id'];
						...
						?>
								...
								<td><?php echo $rs['customer_uid'];?></td>
										<td><?php echo $rs['customer_first_name'];?></td>
										<td><?php echo "OD".$order_id."_R".$rs['id'];?></td>
										<td><?php echo $rs['final_total'];?></td>
										<td><?php echo "net banking - renewal payment";?></td>
										
										<td><?php echo date("d-m-Y h:i:s",strtotime($rs['ts']));?></td>
                                        <td><?php echo $rs['amount'];?></td>
                                        <td><?php echo $rs['final_status'];?></td>
