...
<table cellpadding="0" cellspacing="3" border="0" class="table table-striped table-bordered" id="example">
									...
									<?php 
									$slno=1;
									$q=mysql_query("SELECT * FROM `qualitycheck` q left join gps_model_details g on g.model_id=q.model_id where q.status='rejected' order by q.id desc");
								...
								$devid=$res['device_id'];
									...
									?>
										...
										<td><?php echo $res['slnumber'];?></td>
											<td><div class="hidden-phone visible-desktop action-buttons"><span class="label label-important"><?php echo $res['status'];?></span></div></td>
